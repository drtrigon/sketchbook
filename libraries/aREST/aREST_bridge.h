...
void process(YunClient client) {
  // 'GET /arduino/digital/13 ...'
//  String command = client.readStringUntil('/');
  String command;
  command = client.readStringUntil(' ');  // throw away 'GET '
  //Console.println( client.readStringUntil(' ') );
  command = client.readStringUntil('/');  // throw away '/'
  command = client.readStringUntil('/');
  if (command.indexOf("arduino") > -1) {
    command = client.readStringUntil('/');
  }
...
void digitalCommand(BridgeClient client) {
  ...
  if (client.read() == '/') {
    ...
    client.print(F("\"mode\": \"write\", "));
  } else {
    value = digitalRead(pin);
    
    client.print(F("\"mode\": \"read\", "));
  }
...
/*    client.print(F("Pin D"));
    ...
    client.print(pin);
    client.print(F(", "));
    client.print(F("\"mode\": \"output\", "));
    client.println(F("}\""));
    return;
  }

//  client.print(F("error: invalid mode "));
  client.print(F("\"message\": \"error: invalid mode '"));
  client.print(mode);
  client.print(F("'\", "));
  client.println(F("}"));
}
...
//virtual void root_answer() {
virtual void root_answer(BridgeClient client) {

    // Start
    client.print(F("{\"variables\": {"));
    ...
    if (variables_index == 0 && string_variables_index == 0 && float_variables_index == 0){
      ...
      if (variables_index > 0){
        ...
        for (uint8_t i = 0; i < variables_index; i++){
          client.print(F("\""));
          client.print(int_variables_names[i]);
          client.print(F("\": "));
//          client.print(*int_variables[i]);
          int value;;
          if (int_variables[i]) {  // is NULL?
            value = *int_variables[i];
          }
          else {
            char data[10];
            // Retrieve value from datastore by key
            // https://www.arduino.cc/en/Reference/YunGet
            Bridge.get(int_variables_names[i], data, 10);
            value = String(data).toInt();
          }
          client.print(value);
          client.print(F(", "));
        }
      ...
      if (string_variables_index > 0){
        ...
        for (uint8_t i = 0; i < string_variables_index; i++){
          client.print(F("\""));
          client.print(string_variables_names[i]);
          client.print(F("\": \""));
//          client.print(*string_variables[i]);
          String value;;
          if (string_variables[i]) {  // is NULL?
            value = *string_variables[i];
          }
          else {
            char data[10];
            // Retrieve value from datastore by key
            // https://www.arduino.cc/en/Reference/YunGet
            Bridge.get(string_variables_names[i], data, 10);
            value = String(data);
          }
          client.print(value);
          client.print(F("\", "));
      ...
      if (float_variables_index > 0){
        ...
        for (uint8_t i = 0; i < float_variables_index; i++){
          client.print(F("\""));
          client.print(float_variables_names[i]);
          client.print(F("\": "));
//          client.print(*float_variables[i]);
          float value;;
          if (float_variables[i]) {  // is NULL?
            value = *float_variables[i];
          }
          else {
            char data[10];
            // Retrieve value from datastore by key
            // https://www.arduino.cc/en/Reference/YunGet
            Bridge.get(float_variables_names[i], data, 10);
            value = String(data).toFloat();
          }
          client.print(value);
          client.print(F(", "));
        }
...
//      if (variables_index > 0){
        ...
        for (uint8_t i = 0; i < variables_index; i++){
          if (strcmp(variable_name, int_variables_names[i]) == 0) {
            return true;
          }
        }
...
//      if (string_variables_index > 0){
        ...
        for (uint8_t i = 0; i < string_variables_index; i++){
          if (strcmp(variable_name, string_variables_names[i]) == 0) {
            return true;
          }
        }
...
//      if (float_variables_index > 0){
        ...
        for (uint8_t i = 0; i < float_variables_index; i++){
          if (strcmp(variable_name, float_variables_names[i]) == 0) {
            return true;
          }
        }
...
void set_id(char *device_id){
  ...
  if (DEBUG_MODE) {
    Serial.print(F("Input MQTT topic: "));
    Serial.println(in_topic);

    Serial.print(F("Output MQTT topic: "));
