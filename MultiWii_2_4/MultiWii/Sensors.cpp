...
// ************************************************************************************************************
// I2C Barometer BOSCH BME280 (DrTrigon) (+Temperature +Humidity)
// ************************************************************************************************************
// I2C adress: 0x77 (7bit)
// principle:
//  1) read the calibration register (only once at the initialization)
...
//  4) raw temp + raw pressure => calculation of the adjusted pressure
//  the following code uses the maximum precision setting (oversampling setting 3)
// SUPPORTS humidity measurements also!
// ************************************************************************************************************
...
// ??? compare to bme280 datasheet ???
static struct {
  // sensor registers from the BOSCH BMP085 datasheet
  union {int32_t val; int8_t raw[4]; } ut; //uncompensated T
  union {int32_t val; int8_t raw[4]; } up; //uncompensated P
  uint8_t  state;
  uint32_t deadline;
} bme280_ctx;
...
//void i2c_BME280_readCalibration(){
void i2c_BME280_readCoefficients(void)
{
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_T1, (uint8_t*)&(_bme280_calib.dig_T1), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_T2, (uint8_t*)&(_bme280_calib.dig_T2), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_T3, (uint8_t*)&(_bme280_calib.dig_T3), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P1, (uint8_t*)&(_bme280_calib.dig_P1), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P2, (uint8_t*)&(_bme280_calib.dig_P2), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P3, (uint8_t*)&(_bme280_calib.dig_P3), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P4, (uint8_t*)&(_bme280_calib.dig_P4), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P5, (uint8_t*)&(_bme280_calib.dig_P5), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P6, (uint8_t*)&(_bme280_calib.dig_P6), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P7, (uint8_t*)&(_bme280_calib.dig_P7), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P8, (uint8_t*)&(_bme280_calib.dig_P8), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_P9, (uint8_t*)&(_bme280_calib.dig_P9), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H1, &(_bme280_calib.dig_H1), 1);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H2, (uint8_t*)&(_bme280_calib.dig_H2), 2);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H3, &(_bme280_calib.dig_H3), 1);
  uint8_t temp1, temp2;
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H4, &temp1, 1);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H4+1, &temp2, 1);
  _bme280_calib.dig_H4 = (temp1 << 4) | (temp2 & 0xF);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H5+1, &temp1, 1);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H5, &temp2, 1);
  _bme280_calib.dig_H5 = (temp1 << 4) | (temp2 >> 4);
  i2c_read_reg_to_buf(BME280_ADDRESS, BME280_REGISTER_DIG_H6, (uint8_t*)&(_bme280_calib.dig_H6), 1);
}
...
void i2c_BME280_setSampling(void) {
  ...
  i2c_writeReg(BME280_ADDRESS, BME280_REGISTER_CONTROLHUMID, _humReg.get());
  i2c_writeReg(BME280_ADDRESS, BME280_REGISTER_CONFIG, _configReg.get());
  i2c_writeReg(BME280_ADDRESS, BME280_REGISTER_CONTROL, _measReg.get());
}

// read uncompensated temperature value: ??? send command first
void i2c_BME280_UT_Start(void) {
  //i2c_writeReg(BMP085_ADDRESS,0xf4,0x2e);
  i2c_rep_start(BME280_ADDRESS<<1); //I2C write direction => 0
  i2c_write(BME280_REGISTER_TEMPDATA);
  i2c_stop();
}
...
void i2c_BME280_UT_Read() {
  i2c_rep_start((BME280_ADDRESS<<1) | 1);//I2C read direction => 1
  bme280_ctx.ut.raw[2] = i2c_readAck();
  bme280_ctx.ut.raw[1] = i2c_readAck();
  bme280_ctx.ut.raw[0] = i2c_readNak();
}
...
void i2c_BME280_UP_Start () {
  //i2c_writeReg(BMP085_ADDRESS,0xf4,0x34+(OSS<<6)); // control register value for oversampling setting 3
  i2c_rep_start(BME280_ADDRESS<<1); //I2C write direction => 0
  i2c_write(BME280_REGISTER_PRESSUREDATA);
  i2c_stop();
}
...
void i2c_BME280_UP_Read () {
  i2c_rep_start((BME280_ADDRESS<<1) | 1);//I2C read direction => 1
  bme280_ctx.up.raw[2] = i2c_readAck();
  bme280_ctx.up.raw[1] = i2c_readAck();
  bme280_ctx.up.raw[0] = i2c_readNak();
}
...
void i2c_BME280_Calculate() {
  ...
  // Temperature calculations
  bme280_ctx.ut.val >>= 4;
  x1 = (((((int32_t)bme280_ctx.ut.val>>3) - ((int32_t)_bme280_calib.dig_T1 <<1))) *
         ((int32_t)_bme280_calib.dig_T2)) >> 11;
  x2 = ((((((int32_t)bme280_ctx.ut.val>>4) - ((int32_t)_bme280_calib.dig_T1)) *
          (((int32_t)bme280_ctx.ut.val>>4) - ((int32_t)_bme280_calib.dig_T1))) >> 12) *
          ((int32_t)_bme280_calib.dig_T3)) >> 14;
  //t_fine = x1 + x2;
  b5 = x1 + x2;
  //float T = (t_fine * 5 + 128) >> 8;
  baroTemperature = (b5 * 5 + 128) >> 8; // in 0.01 degC (same as BMP085 temperature)
  // Pressure calculations
  bme280_ctx.up.val >>= 4;
  y1 = ((int64_t)b5) - 128000;
  y2 = y1 * y1 * (int64_t)_bme280_calib.dig_P6;
  y2 = y2 + ((y1*(int64_t)_bme280_calib.dig_P5)<<17);
  y2 = y2 + (((int64_t)_bme280_calib.dig_P4)<<35);
  y1 = ((y1 * y1 * (int64_t)_bme280_calib.dig_P3)>>8) +
       ((y1 * (int64_t)_bme280_calib.dig_P2)<<12);
  y1 = (((((int64_t)1)<<47)+y1))*((int64_t)_bme280_calib.dig_P1)>>33;
  if (y1 == 0) {
    baroPressure = 0.F; // avoid exception caused by division by zero
  } else {
    p = 1048576 - bme280_ctx.up.val;
    p = (((p<<31) - y2)*3125) / y1;
    y1 = (((int64_t)_bme280_calib.dig_P9) * (p>>13) * (p>>13)) >> 25;
    y2 = (((int64_t)_bme280_calib.dig_P8) * p) >> 19;
    //baroPressure = ((p + y1 + y2) >> 8) + (((int64_t)_bme280_calib.dig_P7)<<4);
    baroPressure = (float)(((p + y1 + y2) >> 8) + (((int64_t)_bme280_calib.dig_P7)<<4))/256; // in Pa
  }
  // Humidity calculations (not implemented yet)
  //baroHumidity = 42.;  // ??? not implemented yet
}
...
//  delay(10);
  ...
  i2c_writeReg(BME280_ADDRESS, BME280_REGISTER_SOFTRESET, 0xB6);
  // wait for chip to wake up.
  delay(300);
  // if chip is still reading calibration, delay
  while ((i2c_readReg(BME280_ADDRESS, BME280_REGISTER_STATUS) & (1 << 0)) != 0)
    delay(100);
  i2c_BME280_readCoefficients(); // read trimming parameters, see DS 4.2.2
  i2c_BME280_setSampling(); // use defaults
  delay(5);
  i2c_BME280_UT_Start();
  bme280_ctx.deadline = currentTime+5000;
}

//return 0: no data available, no computation ;  1: new value available  ; 2: no new value, but computation time
uint8_t Baro_update() {                   // first UT conversion is started in init procedure
  if (currentTime < bme280_ctx.deadline) return 0;
  bme280_ctx.deadline = currentTime+6000; // ??? 1.5ms margin according to the spec (4.5ms T convetion time)
  if (bme280_ctx.state == 0) {
    i2c_BME280_UT_Read();
    i2c_BME280_UP_Start();
    bme280_ctx.state = 1;
    Baro_Common();
    bme280_ctx.deadline += 21000;   // ??? 6000+21000=27000 1.5ms margin according to the spec (25.5ms P convetion time with OSS=3)
    return 1;
  } else {
    i2c_BME280_UP_Read();
    i2c_BME280_UT_Start();
    i2c_BME280_Calculate();
    bme280_ctx.state = 0;
    return 2;
  }
}
...
// ************************************************************************************************************
// Start Of I2C Gyroscope and Accelerometer and Compass LSM9DS0 (DrTrigon) (+Temperature)
// ************************************************************************************************************
// I2C adress: Gyroscope 0x6B (7bit), Accelerometer and Compass (+Temperature) 0x1D (7bit)
// SUPPORTS temperature measurements also!
// ************************************************************************************************************
#if defined(LSM9DS0)
...
void ACC_init () {
  ...
  i2c_writeReg(LSM9DS0_ADDRESS_ACCELMAG, LSM9DS0_REGISTER_CTRL_REG1_XM, 0x67); // 100hz XYZ
  i2c_writeReg(LSM9DS0_ADDRESS_ACCELMAG, LSM9DS0_REGISTER_CTRL_REG5_XM, 0b11110000);
  //delay(5);

  // Set default ranges for the various sensors
  //setupAccel(LSM9DS0_ACCELRANGE_2G);
  uint8_t reg = i2c_readReg(LSM9DS0_ADDRESS_ACCELMAG, LSM9DS0_REGISTER_CTRL_REG2_XM);
  reg &= ~(0b00111000);
...
void ACC_getADC () {
  ...
  i2c_getSixRawADC(LSM9DS0_ADDRESS_ACCELMAG, 0x80 | LSM9DS0_REGISTER_OUT_X_L_A);// Start multiple read at reg LSM9DS0_REGISTER_OUT_X_L_A
...
void Gyro_init() {
  ...
  i2c_writeReg(LSM9DS0_ADDRESS_GYRO, LSM9DS0_REGISTER_CTRL_REG1_G, 0x0F); // on XYZ
  //delay(5);
  // enable the temperature sensor (output rate same as the mag sensor)
  uint8_t tempReg = i2c_readReg(LSM9DS0_ADDRESS_ACCELMAG, LSM9DS0_REGISTER_CTRL_REG5_XM);
  i2c_writeReg(LSM9DS0_ADDRESS_ACCELMAG, LSM9DS0_REGISTER_CTRL_REG5_XM, tempReg | (1<<7));
...
void Gyro_getADC () {
  ...
  //readGyro();
  // Read gyro
  i2c_getSixRawADC(LSM9DS0_ADDRESS_GYRO, 0x80 | LSM9DS0_REGISTER_OUT_X_L_G);// Start multiple read at reg LSM9DS0_REGISTER_OUT_X_L_G
  ...
  // Temperature calculations (not implemented yet)
//  readTemp();
/*  // Read temp sensor
  i2c_read_reg_to_buf(LSM9DS0_ADDRESS_ACCELMAG, 0x80 | LSM9DS0_REGISTER_TEMP_OUT_L_XM, rawADC, 2);

  // Shift values to create properly formed integer (low byte first)
  int16_t temperature = (rawADC[1]<<8) | rawADC[0]);*/
}
////////////////////////////////////
//            Gyro end            //
////////////////////////////////////
...
////////////////////////////////////
  void Mag_init() {
    ...
    i2c_writeReg(LSM9DS0_ADDRESS_ACCELMAG, LSM9DS0_REGISTER_CTRL_REG7_XM, 0b00000000);
    //delay(100);
  ...
  void Device_Mag_getADC() {
    ...
    i2c_getSixRawADC(LSM9DS0_ADDRESS_ACCELMAG, 0x80 | LSM9DS0_REGISTER_OUT_X_L_M);// Start multiple read at reg LSM9DS0_REGISTER_OUT_X_L_M
...
// ************************************************************************************************************
// End Of I2C Gyroscope and Accelerometer and Compass LSM9DS0 (DrTrigon) (+Temperature)
// ************************************************************************************************************
