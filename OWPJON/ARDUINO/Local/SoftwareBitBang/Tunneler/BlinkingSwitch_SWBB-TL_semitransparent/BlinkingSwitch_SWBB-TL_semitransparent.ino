...
//#define ...
#define READ_DEBUG 0x99  // read debug info - need better command value
...
#if defined(ENABLE_DEBUG) || defined(ENABLE_SWBB_DEBUG)
...
//  digitalWrite(ERROR_LED_PIN, HIGH);
...
#endif
    ++count_PJON_PACKETS_BUFFER_FULL;
  }
  if(code == PJON_CONTENT_TOO_LONG) {
    ...
    SERIAL.println(data);
#endif
    ++count_PJON_CONTENT_TOO_LONG;
  }
}
#endif

#if defined(ENABLE_SWBB_DEBUG)
...
//  digitalWrite(LED_BUILTIN, HIGH);
  ...
  case READ_INFO: {
    ...
    bus1.reply(SENSOR, strlen(SENSOR));
  }
  break;
  case READ_DEBUG: {
    float val = count_PJON_CONNECTION_LOST + count_PJON_PACKETS_BUFFER_FULL + count_PJON_CONTENT_TOO_LONG;
// + also count watchdog resets and may be also return packetRssi, packetSnr values*/
    bus1.reply((char*)(&val), sizeof(float));
  }
  break;
  default:
    // nop
