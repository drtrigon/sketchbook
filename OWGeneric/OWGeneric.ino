...
(Emulate generic 1wire slave (owfs compatible); DS28EC20)
...
//    detachPcInterrupt(OWPin);
    ...
    if (ds2450a.presence()) {
        if (ds2450a.recvAndProcessCmd()) {
            uint8_t cmd = ds2450a.recv();
            emuDS2450(cmd);
        }
    }
    if (ds2408a.presence()) {
        if (ds2408a.recvAndProcessCmd()) {
            uint8_t cmd = ds2408a.recv();
            emuDS2408(cmd);
        }
    }
    if (ds2408b.presence()) {
        if (ds2408b.recvAndProcessCmd()) {
            uint8_t cmd = ds2408b.recv();
            emuDS2408(cmd);
        }
    }
    if (ds.presence()) {
