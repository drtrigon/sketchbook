...
0x 01 00 00 00   01 00 MSB LSB   read ??? (A0) and return int / [description???]
0x 02 00 00 00   02 00 MSB LSB   read ??? (A1) and return int / [description???]
0x 03 00 00 00   03 00 MSB LSB   read ??? (A2) and return int / [description???]
//0x 04 00 00 00   04 00 MSB LSB   read ??? (A3) and return int / [description???]
...
//    One Wire Slave Data
//                          {Fami, <---, ----, ----, ID--, ----, --->,  CRC} 
unsigned char rom[8]      = {0x24, 0xE2, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00};
...
void process(){
    ...
    } else if (rtccountr_in[1] == 0x01) {  // A0
...
//        bytes[1] = (val >> 16) & 0xFF;
        ...
        rtccountr_out[3] = (val >> 8) & 0xFF;
        rtccountr_out[4] = val & 0xFF;
    } else if (rtccountr_in[1] == 0x02) {  // A1
        unsigned int val = analogRead(A1);
        rtccountr_out[1] = rtccountr_in[1];
        rtccountr_out[2] = 0x00;
        rtccountr_out[3] = (val >> 8) & 0xFF;
        rtccountr_out[4] = val & 0xFF;
    } else if (rtccountr_in[1] == 0x03) {  // A2
        unsigned int val = analogRead(A2);
        rtccountr_out[1] = rtccountr_in[1];
        rtccountr_out[2] = 0x00;
        rtccountr_out[3] = (val >> 8) & 0xFF;
        rtccountr_out[4] = val & 0xFF;
//    } else if (rtccountr_in[1] == 0x04) {  // A3
//      ...
    }
